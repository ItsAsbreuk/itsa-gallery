 ...
 */
    ...
    */
    initializer: function () {
        Y.log('initializer', 'info', 'Itsa-Storage');
        var instance = this,
            w = Y.config.win;
...
//==================================================================================================
        ...
        if (instance.storageMode === instance.MODE_DB) {
            Y.log('initializer setting up MODE_DB', 'info', 'Itsa-Storage');
                        ...
                        t.executeSql(
                            ...
                            Y.bind(
                                function (t, results) {
                                    ...
                                    Y.log('initializer MODE_DB ready', 'info', 'Itsa-Storage');
                                },
                                instance
                            )
                        );
        ...
        if (instance.storageMode === instance.MODE_USERDATA) {
            Y.log('initializer setting up MODE_USERDATA', 'info', 'Itsa-Storage');
            ...
            Y.on(
                ...
                Y.bind(
                    function () {
                        ...
                        Y.fire(instance.EVT_READY);
                        Y.log('initializer MODE_USERDATA ready', 'info', 'Itsa-Storage');
                    },
                    instance
                )
            );
    ...
    clear: function (callback, context, args) {
        var instance = this;
        Y.log('clear storage', 'info', 'Itsa-Storage');
    ...
    _clearLocal: function (callback, context, args) {
        var instance = this,
            err = null;
        Y.log('_clearLocal using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        var key;
        try {
            if (instance.storageMode === instance.MODE_HTML5) {
        ...
        if ((instance.storageMode === instance.MODE_DB) || (instance.storageMode === instance.MODE_USERDATA)) {
            if (instance.initialised_DB_USER_DATA) {
                Y.log('_clearLocal MODE_DB/MODE_USERDATA straight ahead', 'info', 'Itsa-Storage');
                instance.data = {};
                instance._setItemLocal_OldBrowsers(null, null, callback, context, args);
            }
            else {
                Y.log('_clearLocal MODE_DB/MODE_USERDATA but only after waiting for ready event', 'info', 'Itsa-Storage');
                Y.on(
                    instance.EVT_READY,
                    function() {
                        instance.data = {};
    ...
    _clearServer: function (callback, context, args, localError) {
        Y.log('_clearServer', 'info', 'Itsa-Storage');
        var instance = this,
            url,
            stringifyItems;
        if (!instance._serverBkpExists) {
            Y.log('_clearServer stopped executing: _serverBkpExists tells there is no serverstorage', 'info', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
        ...
        if (instance.server) {
            if (instance._storageCookieEnabled()) {
                ...
                Y.log('Cookie-errror: Cookies are disabled: cannot contact server', 'warn', 'Itsa-Storage');
                if (Y.Lang.isFunction(callback)) {
            ...
            Y.log('_clearOnServer server or uriSetItem not defined', 'warn', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
    ...
    getItem: function (key, callback, context, args) {
        var instance = this;
        Y.log('getItem key '+key, 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_SERVER) {
                ...
                instance._getItemLocal(
                    ...
                    function(err, data) {
                        if (!err && data) {
                            ...
                            Y.log('getItem not locally found: try to get it on server', 'info', 'Itsa-Storage');
    ...
    _getItemLocal: function (key, callback, context, args) {
        var instance = this,
            ...
            err = null,
            storagevalue;
        Y.log('_getItemLocal key: '+key+' using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        if ((instance.storageMode === instance.MODE_HTML5) || (instance.storageMode === instance.MODE_GECKO)) {
            ...
            if (instance.initialised_DB_USER_DATA) {
                Y.log('getItem returned straight ahead', 'info', 'Itsa-Storage');
                ...
                Y.log('getItem after waited for ready event', 'info', 'Itsa-Storage');
                Y.on(
                    instance.EVT_READY,
                    function() {
                        var instance = this,
    ...
    _getItemServer: function (key, callback, context, args, localError) {
        Y.log('_getItemServer key: '+key, 'info', 'Itsa-Storage');
        var instance = this,
            url,
            stringifyItems;
        if (!instance._serverBkpExists) {
            Y.log('_getItemServer stopped executing: _serverBkpExists tells there is no serverstorage', 'info', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
        ...
        if (instance.server) {
            ...
            Y.log('_getItemOnServer server or uriSetItem not defined', 'warn', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
    ...
    getItems: function (keys, callback, context, args) {
        var instance = this;
        Y.log('getItems', 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_SERVER) {
                ...
                instance._getItemsLocal(
                    ...
                    function(err, data, notfoundkeys) {
                        if (!err && data && !Y.Lang.isValue(notfoundkeys)) {
                            ...
                            Y.log('getItems not all are locally found: try to get it on server', 'info', 'Itsa-Storage');
    ...
    _getItemsLocal: function (keys, callback, context, args) {
        ...
        Y.log('_getItemsLocal using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        if ((instance.storageMode === instance.MODE_NOOP) || (instance.storageMode === instance.MODE_SERVER) || !isarray) {
            if (!isarray) {
                Y.log('_getItemsLocal keys: '+keys+' if not an array', 'warn', 'Itsa-Storage');
            }
            if (Y.Lang.isFunction(callback)) {
    ...
    _getItemsServer: function (keys, callback, context, args, localError, localData) {
        Y.log('_getItemServer keys: '+keys, 'info', 'Itsa-Storage');
        var instance = this,
            url,
            isarray = Y.Lang.isArray(keys),
            stringifyItems;
    ...
    length: function (callback, context, args) {
        var instance = this;
        Y.log('length', 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_SERVER) {
    ...
    _lengthLocal: function (callback, context, args) {
        var instance = this,
            ...
            err = null,
            key;
        Y.log('_lengthLocal using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_HTML5 || instance.storageMode === instance.MODE_GECKO) {
            try {
            ...
            if (instance.initialised_DB_USER_DATA) {
                ...
                Y.log('length returned straight ahead: '+count, 'info', 'Itsa-Storage');
                Y.rbind(callback, context, null, count, args)();
            }
            else {
                Y.log('length after waited for ready event', 'info', 'Itsa-Storage');
                Y.on(
                    instance.EVT_READY,
                    function() {
                        var key;
                        ...
                        Y.log('length after waited for ready event MODE_DB: '+count, 'info', 'Itsa-Storage');
                        Y.rbind(callback, context, null, count, args)();
                    },
                    instance
                );
    ...
    _lengthServer: function (callback, context, args, localLength, localError) {
        Y.log('_lengthServer', 'info', 'Itsa-Storage');
        var instance = this,
            url,
            stringifyItems;
        if (!instance._serverBkpExists) {
            Y.log('_lengthServer stopped executing: _serverBkpExists tells there is no serverstorage', 'info', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
        ...
        if (instance.server) {
            ...
            Y.log('_getLengthOnServer server or uriGetLength not defined', 'warn', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
    ...
    removeItem: function (key, callback, context, args) {
        var instance = this;
        Y.log('removeItem', 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_SERVER) {
    ...
    _removeItemLocal: function (key, callback, context, args) {
        var instance = this,
            err = null;
        Y.log('_removeItemLocal using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_DB || instance.storageMode === instance.MODE_USERDATA) {
            if (instance.initialised_DB_USER_DATA) {
                Y.log('_removeItemLocal MODE_DB/MODE_USERDATA straight ahead', 'info', 'Itsa-Storage');
                delete instance.data[key];
                instance._setItemLocal_OldBrowsers(null, null, callback, context, args);
            }
            else {
                Y.log('_removeItemLocal MODE_DB/MODE_USERDATA but only after waiting for ready event', 'info', 'Itsa-Storage');
                Y.on(
                    instance.EVT_READY,
                    function() {
                        delete instance.data[key];
    ...
    _removeItemServer: function (key, callback, context, args, localError) {
        Y.log('_removeItemServer key: '+key, 'info', 'Itsa-Storage');
        var instance = this,
            url,
            isarray = Y.Lang.isArray(key),
            stringifyItems;
        if (!instance._serverBkpExists) {
            Y.log('_removeItemServer stopped executing: _serverBkpExists tells there is no serverstorage', 'info', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
                Y.rbind(callback, context, null, args)();
            }
            return;
        ...
        if (instance.server) {
            ...
            Y.log('_removeItemOnServer server or uriSetItem not defined', 'warn', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
                Y.rbind(callback, context, (localError ? localError + ' + ' : '') + 'setRemoteStorageServer is not set in the right way', args)();
            }
        }
    ...
    removeItems: function (keys, callback, context, args) {
        var instance = this;
        Y.log('removeItems', 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_SERVER) {
    ...
    _removeItemsLocal: function (keys, callback, context, args) {
        var instance = this,
            isarray = Y.Lang.isArray(keys),
            err = null;
        Y.log('_removeItemsLocal using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        if (!isarray) {
            Y.log('_getItemsLocal keys: '+keys+' if not an array', 'warn', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
        ...
        if (instance.storageMode === instance.MODE_DB || instance.storageMode === instance.MODE_USERDATA) {
            if (instance.initialised_DB_USER_DATA) {
                Y.log('_removeItemsLocal MODE_DB/MODE_USERDATA straight ahead', 'info', 'Itsa-Storage');
                Y.Array.each(
                    keys,
                ...
                Y.log('_removeItemsLocal MODE_DB/MODE_USERDATA but only after waiting for ready event', 'info', 'Itsa-Storage');
                Y.on(
                    instance.EVT_READY,
                    function() {
                        Y.Array.each(
    ...
    setItem: function (key, value, callback, context, args) {
        var instance = this;
        Y.log('setItem key '+key, 'info', 'Itsa-Storage');
        if (!Y.Lang.isString(key)) {
            Y.log('setItem: no valid key given', 'warn', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
        ...
        if (!Y.Lang.isValue(value)) {
            Y.log('setItem: no value given', 'info', 'Itsa-Storage');
    ...
    _setItemLocal: function (key, value, callback, context, args) {
        ...
        if (!transformedValue) {
            Y.log('_setItemLocal key: '+key+' --> value is null: will not be stored', 'info', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
                Y.rbind(callback, context, 'key: '+key+' --> value is null: key is not stored', args)();
            }
            return;
        }
        Y.log('_setItemLocal key: '+key+' --> value '+transformedValue+' --> using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_DB || instance.storageMode === instance.MODE_USERDATA) {
    ...
    _setItemServer: function (key, value, callback, context, args, localError) {
        Y.log('_setItemServer key: '+key+' --> value: '+value, 'info', 'Itsa-Storage');
        var instance = this,
            url,
        ...
        if (instance.server) {
            if (instance._storageCookieEnabled()) {
                ...
                Y.log('_setItemServer calling '+url, 'info', 'Itsa-Storage');
                Y.jsonp(
                    url,
                    {
                        on: {
            ...
            Y.log('_doSaveOnServer server or uriSetItem not defined', 'warn', 'Itsa-Storage');
            if (Y.Lang.isFunction(callback)) {
                Y.rbind(callback, context, (localError ? localError + ' + ' : '') + 'setRemoteStorageServer is not set in the right way', args)();
            }
        }
    ...
    setItems: function (keyvaluepairs, callback, context, args) {
        var instance = this;
        Y.log('setItem keyvaluepairs '+keyvaluepairs, 'info', 'Itsa-Storage');
        if (instance.storageMode === instance.MODE_SERVER) {
    ...
    _setItemsLocal: function (keyvaluepairs, callback, context, args) {
        ...
        if ((instance.storageMode === instance.MODE_NOOP) || (instance.storageMode === instance.MODE_SERVER) || !isarray) {
            if (!isarray) {
                Y.log('_getItemsLocal key: '+keyvaluepairs+' if not an array', 'warn', 'Itsa-Storage');
            }
            if (Y.Lang.isFunction(callback)) {
        ...
        Y.Array.each(
            ...
            function(keyvaluepair, index, keyvaluepairsarray) {
                if (Y.Lang.isArray(keyvaluepair) && (keyvaluepair.length===2)) {
                    ...
                    Y.log('_getItemsLocal '+err, 'warn', 'Itsa-Storage');
                }
            },
            instance
        );
    ...
    _setItemsServer: function (keyvaluepairs, callback, context, args, localError, localSetItems) {
        Y.log('_setItemServer', 'info', 'Itsa-Storage');
        var instance = this,
            url;
        if (!localSetItems) {
            localSetItems = [];
    ...
    setRemoteStorageServer: function(server, uriSetItem, uriGetItem, uriGetLength, uriRemoveItem, uriClear, force, serverTimeout) {
        Y.log('setRemoteStorageServer '+server, 'info', 'Itsa-Storage');
        var instance = this;
        if (Y.Lang.isString(server) && (server!==instance.server)) {
            ...
            Y.log('setRemoteStorageServer '+server+' is not a String', 'warn', 'Itsa-Storage');
        }
    },
    ...
    _getCorrectItemType: function(item) {
        ...
        Y.log('_getCorrectItemType using type: '+type, 'info', 'Itsa-Storage');
        itemremain = item.substr(1);
        if (type === instance.TYPE_STRING) {
            Y.log('_getCorrectItemType type is String', 'info', 'Itsa-Storage');
            returnvalue = itemremain;
        }
        else if (type === instance.TYPE_INT) {
            Y.log('_getCorrectItemType type is Int', 'info', 'Itsa-Storage');
            returnvalue = parseInt(itemremain, 10);
        }
        else if (type === instance.TYPE_FLOAT) {
            Y.log('_getCorrectItemType type is Float', 'info', 'Itsa-Storage');
            returnvalue = parseFloat(itemremain);
        }
        else if (type === instance.TYPE_BOOLEAN) {
            Y.log('_getCorrectItemType type is Boolean', 'info', 'Itsa-Storage');
            returnvalue = (itemremain==='1');
        }
        else if (type === instance.TYPE_ARRAY) {
            try {
                returnvalue = Y.JSON.parse(itemremain);
                Y.log('_getCorrectItemType type is Array', 'info', 'Itsa-Storage');
            }
            catch(e) {
                returnvalue = itemremain;
                Y.log('_getCorrectItemType converting to Array failed --> returning String. Errorcode: '+e.message, 'warn', 'Itsa-Storage');
            }
        }
        else if (type === instance.TYPE_DATE) {
            returnvalue = new Date();
            returnvalue.setTime(itemremain);
            Y.log('_getCorrectItemType type is Date', 'info', 'Itsa-Storage');
        }
            ...
            try {
                ...
                Y.log('_getCorrectItemType type is RegExp', 'info', 'Itsa-Storage');
            }
            catch(e2) {
                returnvalue = itemremain;
                Y.log('_getCorrectItemType converting to RegExp failed --> returning String. Errorcode: '+e2.message, 'warn', 'Itsa-Storage');
            }
        }
        else if (type === instance.TYPE_OBJECT) {
            try {
                returnvalue = Y.JSON.parse(itemremain);
                Y.log('_getCorrectItemType type is Object', 'info', 'Itsa-Storage');
            }
            catch(e3) {
                returnvalue = itemremain;
                Y.log('_getCorrectItemType converting to Object failed --> returning String. Errorcode: '+e3.message, 'warn', 'Itsa-Storage');
            }
        }
        return returnvalue;
    },
    ...
    _setItemLocal_OldBrowsers: function (key, value, callback, context, args) {
        var instance = this;
        if (instance.initialised_DB_USER_DATA) {
            Y.log('_setItemLocal_OldBrowsers straight ahead', 'info', 'Itsa-Storage');
            instance._setItemLocal_OldBrowsers_ReadySafe(key, value, callback, context, args);
        }
        else {
            Y.log('_setItemLocal_OldBrowsers but only after waiting for ready event', 'info', 'Itsa-Storage');
            Y.on(
                instance.EVT_READY,
    ...
    _setItemLocal_OldBrowsers_ReadySafe: function (key, value, callback, context, args) {
        ...
        Y.log('_setItemLocal_OldBrowsers_ReadySafe using storagemode '+instance.storageMode, 'info', 'Itsa-Storage');
        try {
            if (instance.storageMode === instance.MODE_DB) {
    ...
    _storageCookieEnabled: function() {
        ...
        Y.log('_storageCookieEnabled '+returnvalue, 'info', 'Itsa-Storage');
        return returnvalue;
    },
    ...
    _readCookie: function() {
        ...
        Y.log('_readCookie '+instance.COOKIE_ID+' --> value: '+returnvalue, 'info', 'Itsa-Storage');
        return returnvalue;
    },
     ...
     */
    _setCookie: function() {
        Y.log('_setCookie', 'info', 'Itsa-Storage');
        var instance = this,
